...
exports.doRegister = function (req, res, next) {
  ...
  userModel.findOne({username: username}, function (err,result) {
    ...
    // 检查用户名是否存在
    if (result) {
      res.status(201).send({code:201, msg: '用户名已存在'}) // 用户名已存在
    } else {
      // 注册新用户，写入数据库
      ...
      userModel.create({username: username, password: password, email: email, avatar: avatar}, function (err, result) {
        err ? res.status(201).send({code:201, msg: '注册失败'}) : res.status(200).send({code:200, msg: '注册成功'})
      })
    }
  })
}
...
exports.doLogin = function (req, res, next) {
  ...
  userModel.findOne({username: username, password: password}, function (err, result) {
    ...
    if (result) {
      req.session.loginUser = username; // 将用户写入 session
      res.status(200).send({code:200, msg:'登陆成功',username:username})
    } else {
      res.status(201).send({code: 201, msg: '用户名或密码错误'}) // 用户名或密码错误
    }
  })
}
 ...
 ********/
//文章分页
exports.getIndexArticle = function (req, res, next) {
  articleModel.find({}).sort({'_id': -1}).limit(4).exec(function (err, result) {
