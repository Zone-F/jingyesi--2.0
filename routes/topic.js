...
exports.newTopic = function (req, res, next) {
  ...
  const book = [req.body.book, req.body.rebook]
  topicModel.create({
    topic: topic,
    user: user,
    users:user,
...
exports.newTopicUser = (req, res, next) => {
  const newuser = req.session.loginUser
    ...
    if (err) {
      ...
      for (let i = 0; i < len; i++) {
        if (user === result[0].userinfo[i].user) {
          result[0].userinfo[i].articlelist.push({
            title: title,
            body: body
          })
          result[0].save(function (err, result) {
...
exports.getTopicOld = (req, res, next) => {
  topicModel.find({}).sort({
    '_id': -1
  }).skip(req.body.page).limit(1).exec(function (err, result) {
    err ? res.status(201).send('-1') : res.status(200).send(result)
  })
}
// 修改活动时间
